#!/usr/bin/env bash
#
# deploy-release.sh — install a pre-built PyMouse binary from GitHub and
# restart the service.
#
# Use this instead of deploy.sh when you want a CI-built binary rather than a
# local compile — typically for rolling back to a past version, or if the Go
# toolchain is unavailable on this host.
#
# The repo is public, so RELEASES download with plain curl — no token, no gh
# login. Only PR-mode (downloading a workflow artifact) needs gh, because
# GitHub gates workflow artifacts behind authentication even on public repos.
#
# Modes (auto-detected from $1):
#
#   scripts/deploy-release.sh             # latest release on PyMouse
#   scripts/deploy-release.sh <tag>       # specific release tag (e.g. 2026-08-04-1cc4e4d)
#   scripts/deploy-release.sh pr          # latest PR workflow artifact (needs gh auth)
#   scripts/deploy-release.sh <run-id>    # specific PR workflow run id (needs gh auth)
#
set -euo pipefail
